...
// Get current content
try {
    $current_query = "SELECT c.id_contenu, c.texte, c.slug, c.id_type, c.id_page, c.id_parent
                      FROM contenu c
    ...
    // Get current tags
    $tags_query = "SELECT id_tag FROM contenu_tag WHERE id_contenu = :id_contenu";
    $tags_stmt = $pdo->prepare($tags_query);
    $tags_stmt->bindParam(':id_contenu', $id_contenu, PDO::PARAM_INT);
    $tags_stmt->execute();
    $current_tags = $tags_stmt->fetchAll(PDO::FETCH_COLUMN);
    
    // Get all available types, pages, and tags
    ...
    $all_tags_query = "SELECT id_tag, libelle FROM tag ORDER BY libelle";
    $all_tags = $pdo->query($all_tags_query)->fetchAll();
} catch (PDOException $e) {
    $error = 'Erreur lors du chargement du contenu: ' . $e->getMessage();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($texte) || empty($slug) || empty($id_type) || empty($id_page)) {
        ...
        try {
            ...
            // Get ordre from form (keep existing if not changed)
            $ordre = !empty($_POST['ordre']) ? (int)$_POST['ordre'] : $content['ordre'];
            
            // Update content
            $update_query = "UPDATE contenu SET texte = :texte, slug = :slug, id_type = :id_type, id_page = :id_page, id_parent = :id_parent, ordre = :ordre
            ...
            $delete_stmt = $pdo->prepare($delete_tags_query);
            $delete_stmt->bindParam(':id_contenu', $id_contenu, PDO::PARAM_INT);
            $delete_stmt->execute();
            ...
            $error = 'Erreur lors de la modification du contenu: ' . $e->getMessage();
        }
    }
}
?>
...
<head>
    ...
    <title>Modifier Contenu - Panneau d'Administration</title>
...
<body>
    <div class="admin-container">
            ...
            <?php endif; ?>            <?php if (!empty($content)): ?>
                <form method="POST" action="" class="admin-form" enctype="multipart/form-data">
                    <div class="form-group">
                        ...
                        <textarea id="texte" name="texte" required rows="10"><?php echo htmlspecialchars($content['texte']); ?></textarea>
                    </div>

                    <div class="form-group">
                        <label for="slug">Slug *</label>
                        <input type="text" id="slug" name="slug" required value="<?php echo htmlspecialchars($content['slug']); ?>">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="id_type" name="id_type">
                            ...
                            <?php foreach ($types as $type): ?>
                                <option value="<?php echo $type['id_type']; ?>" <?php echo $content['id_type'] == $type['id_type'] ? 'selected' : ''; ?>>
                        ...
                        <select id="id_page" name="id_page" required onchange="updateParentOptions()">
                            ...
                            <?php foreach ($pages as $page): ?>
                                <option value="<?php echo $page['id_page']; ?>" <?php echo $content['id_page'] == $page['id_page'] ? 'selected' : ''; ?>>
                    ...
                    <!-- Parent selection - shows dynamically based on type -->
                    <div class="form-group" id="parent-group" style="display: none;">
                        ...
                        <input type="number" id="ordre" name="ordre" value="<?php echo htmlspecialchars($content['ordre'] ?? 0); ?>" min="0" step="1">
                        <small>0 = premier, 1 = deuxième, etc.</small>
                    </div>
                            ...
                            <?php foreach ($all_tags as $tag): ?>
                                <label class="checkbox-label">
                                    <input type="checkbox" name="tags[]" value="<?php echo $tag['id_tag']; ?>" 
                                        <?php echo in_array($tag['id_tag'], $current_tags) ? 'checked' : ''; ?>>
        ...
        const currentParentId = <?php echo json_encode($content['id_parent']); ?>;
        ...
        function updateParentOptions() {
            ...
            if (typeName.includes('Titre Principal')) {
                ...
                mainTitles.forEach(title => {
                    ...
                    if (title.id_contenu == currentParentId) option.selected = true;
                ...
                subTitles.forEach(subtitle => {
                    ...
                    if (subtitle.id_contenu == currentParentId) option.selected = true;
                    parentSelect.appendChild(option);
                });
